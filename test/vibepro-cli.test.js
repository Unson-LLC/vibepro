...
test('INV-001 INV-002 INV-003 C-001 C-002 S-001 design-modernize plan creates Design Cognition Loop evidence and explicit gate checks', async () => {
  ...
  assert.equal(result.result.plan.journey_context.model, 'vibepro-design-modernize-journey-context-v1');
  assert.equal(result.result.plan.journey_context.generated_by, 'design-modernize_plan');
  assert.equal(result.result.plan.journey_context.artifact_kind, 'journey_context_pack');
  assert.equal(result.result.plan.journey_context.curation_status, 'needs_curated_journey');
  assert.equal(result.result.plan.journey_context.curated, false);
  assert.equal(result.result.plan.journey_context.gate.status, 'needs_review');
  assert.match(result.result.plan.journey_context.next_commands[0], /vibepro journey handoff/);
  assert.equal(result.result.plan.design_quality_dag.nodes[0].id, 'design:journey_context');
  assert.equal(result.result.plan.design_quality_dag.edges[0].from, 'design:journey_context');
  assert.ok(result.result.plan.spec_gate.checks.some((check) => check.id === 'JOURNEY-GLOBAL-1'));
  ...
  assert.equal(await pathExists(path.join(repo, '.vibepro', 'design-modernize', 'story-aitle-ds-modernize', 'journey-context.json')), true);
  assert.equal(await pathExists(path.join(repo, '.vibepro', 'journey', 'latest-journey.json')), true);
  assert.equal(await pathExists(path.join(repo, '.vibepro', 'journey', 'latest-handoff.md')), true);
  ...
  const planMarkdown = await readFile(path.join(repo, '.vibepro', 'design-modernize', 'story-aitle-ds-modernize', 'design-modernize.md'), 'utf8');
  ...
  const journeyContext = await readJson(path.join(repo, '.vibepro', 'design-modernize', 'story-aitle-ds-modernize', 'journey-context.json'));
  ...
  assert.equal(journeyContext.authority, 'handoff_context_only');
  assert.equal(journeyContext.curation_status, 'needs_curated_journey');
  assert.match(journeyContext.next_commands[0], /vibepro journey handoff/);
  assert.match(planMarkdown, /## Journey Context/);
  assert.match(planMarkdown, /Curation status: needs_curated_journey/);
  assert.match(planMarkdown, /Next: `vibepro journey handoff/);
  assert.match(spec, /INV-HOME-1/);
  assert.match(spec, /Journey Context/);
  assert.match(spec, /Curation status: needs_curated_journey/);
  assert.match(spec, /Next commands: `vibepro journey handoff/);
  assert.match(spec, /JOURNEY-GLOBAL-1/);
...
test('check self-dogfood accepts GitHub PRs with VibePro body and matching pr-create evidence', async () => {
  ...
  const fakeGh = await makeFakeGh({
    ...
    body: [
      '## 判断',
      '- このPRで判断すること: VibePro経由のPRとして受け入れてよいか。',
      '- Story: story-pr-path',
      '',
      '## 経緯',
      '- 要求: VibePro経由のPRとして受け入れてよいか。',
      '',
      '## 原因',
      '- PR証跡がVibePro経由で閉じているか判断できない。',
      '',
      '## 解決',
      '- VibePro PR create証跡をPR本文へ接続する。',
      '',
      '## レビュー観点',
      '- Gate: required gates are closed',
      '',
      '## 確認',
      '- [x] npm test',
      '- 最終E2E: passed',
      ...
      '- 実行状態: ready',
      '- 証跡: .vibepro/pr/story-pr-path/',
      '- 判断索引: .vibepro/pr/story-pr-path/decision-index.json'
    ].join('\n')
  });
...
test('check self-dogfood rejects failed pr-create evidence for visible GitHub PRs', async () => {
  ...
  const fakeGh = await makeFakeGh({
    ...
    body: [
      '## 判断',
      '- このPRで判断すること: 失敗証跡をPR作成証跡として扱わないか。',
      '- Story: story-pr-path',
      '',
      '## 経緯',
      '- 要求: 失敗証跡をPR作成証跡として扱わないか。',
      '',
      '## 原因',
      '- PR作成証跡の失敗状態が本文だけでは判断できない。',
      '',
      '## 解決',
      '- 失敗したpr-create artifactをGateで拒否する。',
      '',
      '## レビュー観点',
      '- Gate: required gates are closed',
      '',
...
test('check self-dogfood rejects dry-run pr-create evidence for visible GitHub PRs', async () => {
  ...
  const fakeGh = await makeFakeGh({
    ...
    body: [
      '## 判断',
      '- このPRで判断すること: dry-run証跡をPR作成証跡として扱わないか。',
      '- Story: story-pr-path',
      '',
      '## 経緯',
      '- 要求: dry-run証跡をPR作成証跡として扱わないか。',
      '',
      '## 原因',
      '- dry-runのまま見えるPRは作成境界を通った証拠にならない。',
      '',
      '## 解決',
      '- 実PRでは非dry-runのpr-create artifactを要求する。',
      '',
      '## レビュー観点',
      '- Gate: required gates are closed',
      '',
...
test('check self-dogfood rejects stale pr-create evidence for visible GitHub PRs', async () => {
  ...
  const fakeGh = await makeFakeGh({
    ...
    body: [
      '## 判断',
      '- このPRで判断すること: stale証跡をPR作成証跡として扱わないか。',
      '- Story: story-pr-path',
      '',
      '## 経緯',
      '- 要求: stale証跡をPR作成証跡として扱わないか。',
      '',
      '## 原因',
      '- 古いheadのpr-create artifactでは現PRを判断できない。',
      '',
      '## 解決',
      '- 現headに一致するpr-create artifactを要求する。',
      '',
      '## レビュー観点',
      '- Gate: required gates are closed',
      '',
...
`);
  ...
  const prBody = await readFile(result.result.artifacts.pr_body, 'utf8');
  assert.match(prBody, /- 正本: Story未検出/);
  assert.doesNotMatch(prBody, /Story Source story_source_mismatch/);
  assert.doesNotMatch(prBody, /search_wiki に project_id を追加する/);
});

test('pr prepare accepts changed Story docs that explicitly relate to the selected Story', async () => {
  const repo = await makeGitRepoWithStory();
  const storyDir = path.join(repo, 'docs', 'stories');
  await mkdir(storyDir, { recursive: true });
  await writeFile(path.join(storyDir, 'story-visual-runner.md'), `---
story_id: story-visual-runner
title: Visual residual local runner
related_stories:
  - story-pr-prepare
---

# Visual residual local runner

## Acceptance Criteria

- Residual Visual QA evidence can be prepared alongside PR preparation.
`);
  await mkdir(path.join(repo, 'src'), { recursive: true });
  await writeFile(path.join(repo, 'src', 'visual-runner.js'), 'export const visualRunner = true;\n');
  ...
  assert.equal(integrity.mismatched_changed_story_docs.length, 0);
  const relatedDoc = integrity.changed_story_docs.find((doc) => doc.path === 'docs/stories/story-visual-runner.md');
  assert.equal(relatedDoc.matches_selected_story, true);
...
PR本文が証跡索引に寄りすぎていて、Story解釈、発生経緯、原因、解決、最終確認が一画面でつながらない。

## 解決

PR本文を日本語の判断ブリーフにして、詳細artifactではなくレビュー判断に必要な流れを先に出す。
...
`);
  ...
  assert.ok(prBody.indexOf('## 判断') < prBody.indexOf('## 経緯'));
  assert.ok(prBody.indexOf('## 経緯') < prBody.indexOf('## 原因'));
  assert.ok(prBody.indexOf('## 原因') < prBody.indexOf('## 解決'));
  assert.ok(prBody.indexOf('## 解決') < prBody.indexOf('## レビュー観点'));
  assert.ok(prBody.indexOf('## レビュー観点') < prBody.indexOf('## 確認'));
  assert.ok(prBody.indexOf('## 確認') < prBody.indexOf('## 詳細'));
  assert.doesNotMatch(prBody, /^## What$/m);
  assert.doesNotMatch(prBody, /^## Why$/m);
  assert.doesNotMatch(prBody, /^## How to review$/m);
  assert.doesNotMatch(prBody, /^## Verification$/m);
  assert.match(prBody, /変更範囲: \d+ files \/ Runtime \/ Contract Docs \/ Tests/);
  assert.match(prBody, /Gate: 未解決Gateがあります（対象: .*Gate/);
  ...
  assert.doesNotMatch(prBody, /^## 分割計画$/m);
  assert.ok(Buffer.byteLength(prBody, 'utf8') < 20000);
  assert.doesNotMatch(prBody, /MAE 13\.41%/);
  assert.doesNotMatch(prBody, /^## Completion Quality$/m);
...
`);
  ...
  const prBody = await readFile(path.join(repo, '.vibepro', 'pr', 'story-pr-prepare', 'pr-body.md'), 'utf8');
  assert.match(prBody, /Requirement Gateが contradicted/);
  assert.match(prBody, /\.vibepro\/pr\/story-pr-prepare\/pr-prepare\.json/);
  assert.doesNotMatch(prBody, /Potential Contradiction/);
  assert.doesNotMatch(prBody, /Requirement Source: spec:docs\/specs\/billing-subscription\.md/);
});
...
`);
  ...
  const prBody = await readFile(path.join(repo, '.vibepro', 'pr', 'story-pr-prepare', 'pr-body.md'), 'utf8');
  assert.match(prBody, /\.vibepro\/pr\/story-pr-prepare\/pr-prepare\.json/);
  assert.doesNotMatch(prBody, /Spec Sources: 1/);
  assert.doesNotMatch(prBody, /Architecture Sources: 1/);
});
...
test('pr prepare recommends a clean branch for broad session diffs', async () => {
  ...
  const prBody = await readFile(path.join(repo, '.vibepro', 'pr', 'story-pr-prepare', 'pr-body.md'), 'utf8');
  assert.match(prBody, /- Scope: 差分範囲の説明または分割判断が必要/);
  assert.match(prBody, /- Scope: needs_clean_branch \//);
  assert.match(prBody, /- Scope: needs_clean_branch \//);
  assert.doesNotMatch(prBody, /## 監査ログ/);
