...
test('help command prints discoverable usage', async () => {
  ...
  assert.match(output, /vibepro review record \[repo\].*--inspection-input <ref>/);
  ...
  assert.match(output, /vibepro review prepare \[repo\].*--role <role>/);
...
test('skills commands list install and verify bundled VibePro skills', async () => {
  ...
  assert.equal(listResult.result.skills.length, 10);
  ...
  assert.equal(listResult.result.skills.some((skill) => skill.name === 'vibepro-judgment-investigation'), true);
  ...
  assert.equal(await pathExists(path.join(repo, '.claude', 'skills', 'vibepro-judgment-investigation', 'SKILL.md')), false);
  ...
  const judgmentInvestigationSkillPath = path.join(repo, '.claude', 'skills', 'vibepro-judgment-investigation', 'SKILL.md');
  ...
  assert.match(await readFile(judgmentInvestigationSkillPath, 'utf8'), /name: vibepro-judgment-investigation/);
  assert.match(await readFile(judgmentInvestigationSkillPath, 'utf8'), /vibepro judgment investigate/);
  assert.match(await readFile(judgmentInvestigationSkillPath, 'utf8'), /Graphify/);
...
test('beta.27 release metadata and CLI report the exact candidate version', async () => {
  const expectedVersion = '0.2.0-beta.27';
  const packageJson = await readJson(path.resolve('package.json'));
...
test('env graph command runs end-to-end and derives an environment graph', async () => {
 ...
 test('lightweight review CLI records inspected content and rejects missing evidence', async () => {
  const repo = await makeGitRepoWithStory();
  const prepared = await coreRunCli(['review', 'prepare', repo, '--id', 'story-pr-prepare', '--json']);
  assert.equal(prepared.exitCode, 0);
  assert.deepEqual(prepared.result.plan.roles, ['reviewer']);
  const missing = await coreRunCli(['review', 'record', repo, '--id', 'story-pr-prepare', '--status', 'pass', '--summary', '確認済み', '--json']);
  assert.notEqual(missing.exitCode, 0);
  const recorded = await coreRunCli(['review', 'record', repo, '--id', 'story-pr-prepare', '--status', 'pass', '--summary', '確認済み', '--inspection-input', 'index.html', '--json']);
  assert.equal(recorded.exitCode, 0);
  const status = await coreRunCli(['review', 'status', repo, '--id', 'story-pr-prepare', '--json']);
  assert.equal(status.exitCode, 0);
  assert.equal(status.result.complete, true);
});
