...
import { loadTargetModel, runArchitectureConformance } from '../src/architecture-conformance.js';
...
test('model_version is read, surfaced on conformance output, and optional', async () => {
...
test('malformed model_version fails loudly instead of degrading to unversioned', async () => {
...
test('orphan assignment ranks consumers over dependencies and excludes dispatcher modules', async () => {
  const root = await makeRepo();
...
test('assignment candidates carry the module dependencies the assignment would induce', async () => {
  const root = await makeRepo();
  const proposal = await runRebaselineProposal(root, { write: false });
  const orphan = proposal.orphan_assignments.find((entry) => entry.file === 'src/orphan.js');
...
test('an assignment that would create a new violating pair is not recommended as machine maintainable', async () => {
...
test('orphan clusters group mutually importing orphans and are flagged human adjudicated', async () => {
  const root = await makeRepo({
    files: {
      ...defaultFiles(),
...
test('undeclared dependencies are triaged into declare candidates and must-resolve entries', async () => {
  const root = await makeRepo({
    model: defaultModel({
      modules: [
        { name: 'cli', responsibility: 'entrypoint', paths: ['src/cli.js'] },
...
test('declaring a dependency whose reverse is already declared is classified as resolve, not declare', async () => {
  const root = await makeRepo({
    model: defaultModel({
...
test('proposal generation is deterministic apart from generated_at', async () => {
  const root = await makeRepo();
...
test('cli writes proposal artifacts and an optional committed snapshot', async () => {
  const root = await makeRepo();
  const stdout = [];
...
test('the repository target model declares the governance three-way split and a model version', async () => {
