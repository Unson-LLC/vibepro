...
test('story-vibepro-guarded-run-session-contract acceptance and scenario replay', async () => {
  ...
  assert.match(result.stdout, /(?:ℹ|#) pass [1-9]\d*\b/, 'focused contract and process-replay suites must pass before acceptance bindings are evaluated');
  assert.match(result.stdout, /(?:ℹ|#) fail 0\b/, 'focused contract and process-replay suites must have no failures');
