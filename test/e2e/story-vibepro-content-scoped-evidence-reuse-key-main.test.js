...
test(`${STORY} ac:1 ac:2 ac:3 ac:4 ac:5 ac:6 (CRK-S-1 / CRK-S-2 / CRK-S-3 / CRK-S-4 / CRK-S-5 / CRK-S-6) executes the content-scoped evidence reuse key behavior`, async () => {
  ...
  assert.match(stdout, /CRK-S-2 a fix commit touching only one role's inspected surface misses only that role's reuse/);
  ...
  assert.match(stdout, /CRK-S-4 CRK-S-6 spec fingerprint drift invalidates every role's reuse, not only the aggregate status/);
