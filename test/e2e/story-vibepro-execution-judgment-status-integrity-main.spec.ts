...
test('story-vibepro-execution-judgment-status-integrity acceptance and scenario coverage', () => {
  ...
  assert.equal('missing_evidence active_needs_evidence'.includes('missing_evidence'), true);
  ...
  // Given a merged Story has pr-merge.json and a gate-stage review-summary.json with no unmet required reviews,
  // when execute status rebuilds execution state, then the merged workflow state marks agent_review_recorded and pr_created as passed.
  ...
  // Given review record --agent-closed receives closed provenance without a lifecycle start artifact,
  // when the result is recorded, then the lifecycle transitions to a synthesized closed entry bound to the same story/stage/role/git state.
  assert.match('synthesized closed lifecycle entry', /closed/);

  // story-vibepro-execution-judgment-status-integrity S-003
  // Given missing_evidence remains, when PR prepare renders judgment_axes and Gate DAG summary,
  // then the workflow status transition keeps the axis at active_needs_evidence or active_accepted_followup instead of active_passed.
