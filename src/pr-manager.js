...
// has a keyword matcher used to check whether the architecture evidence actually
// addresses it. Deliberately starts narrow: only the workflow/scheduler shape.
...
// Conservative detector: only flag stories that clearly describe scheduled or
// recurring workflow execution. High-precision keywords keep this behind the
// dimension list so we do not block ordinary stories (issue #128 note).
function detectBlueprintShapes(storySource) {
  const text = storyEvidenceText(storySource);
  const shapes = [];
  const schedulerSignal = /(schedul|cron|scheduled job|recurring|every \d|interval|polling|launchd|systemd timer|routine|runner|定期実行|スケジュール|常駐ジョブ)/i.test(text);
  if (schedulerSignal) shapes.push('workflow_scheduler');
  return shapes;
}

// Async: reads architecture doc content + story text and checks which required
// blueprint dimensions are addressed. Returns null when no shape applies.
async function buildArchitectureBlueprintCoverage(repoRoot, { storySource, fileGroups }) {
  ...
  for (const file of (fileGroups.architecture_docs?.files ?? [])) {
    try {
      ...
      // unreadable doc -> treated as no coverage from that file
    }
  }
  const covered = [];
  const missing = [];
