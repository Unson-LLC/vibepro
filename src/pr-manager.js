...
function hasDeployVerificationEvidence({ verificationEvidence = null, decisionRecords = null } = {}) {
  ...
  return commands.some((item) => {
    ...
    if (!['pass', 'passed', 'success', 'ok'].includes(item.status)) return false;
    const haystack = `${item.kind ?? ''}\n${item.command ?? ''}\n${item.summary ?? ''}\n${item.artifact ?? ''}`;
    return /\b(deploy|deployment|rollout|release|smoke|health\s*check|healthcheck|post-?deploy|production verify|prod verify)\b/i.test(haystack);
  });
}
 ...
 * (workflow_heavy / api_contract, or a release/mirror PR route). It does not
 * require the deploy to have happened before merge (pr prepare runs pre-merge);
 * it requires that deploy/verification intent is closed as evidence: a
 * current-bound deploy/smoke/health record, or an explicit waiver decision.
 * "Release Is A Different State" made mechanical, and waivable.
 */
function buildDeployVerificationGate({ environmentGraph, changeClassification, prRoute, verificationEvidence, decisionRecords }) {
