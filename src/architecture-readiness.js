...
import { readArchitectureReadiness, writeArchitectureReadiness } from './architecture-store.js';

const execFileAsync = promisify(execFile);
const SCHEMA_VERSION = '0.1.0';

export async function recordArchitectureReadiness(repoRoot, options = {}) {
  ...
  const readiness = await buildArchitectureReadiness(root, {
    storyId,
    prPrepare: prPrepare.preparation
  });
  const artifactPath = await writeArchitectureReadiness(root, storyId, readiness);
  return {
    readiness: {
      ...readiness,
...
export async function assertArchitectureReadinessForFinal(repoRoot, storyId) {
  const root = path.resolve(repoRoot);
  const readiness = await readArchitectureReadiness(root, storyId);
  if (!readiness) {
    throw new Error(
      `architecture write --final requires Architecture Readiness evidence. ` +
      `Run \`vibepro architecture readiness . --id ${storyId} --base <base-ref>\` first, ` +
      `or write a draft with \`vibepro architecture write --draft\`.`
    );
  }
  ...
  if (currentHead && readiness.git?.head_sha && currentHead !== readiness.git.head_sha) {
    failures.push({
      id: 'current_head',
      status: 'stale',
      reason: `Architecture Readiness was recorded for ${readiness.git.head_sha}, current HEAD is ${currentHead}`
    });
  }
  if (failures.length > 0) {
    throw new Error(
      `architecture write --final blocked by Architecture Readiness: ` +
      failures.map((item) => `${item.id}=${item.status}`).join(', ') +
      `. Rerun \`vibepro architecture readiness . --id ${storyId} --base <base-ref>\`, ` +
      `or write a draft with \`vibepro architecture write --draft\`.`
    );
  }
  return readiness;
...
export function renderArchitectureReadinessSummary(result) {
  ...
  const lines = [
    ...
    `- artifact: ${readiness.artifact ?? '.vibepro/architecture/<story-id>/architecture-readiness.json'}`,
    ''
  ];
...
function buildEngineeringJudgmentCheck(engineeringJudgment, gateDag) {
  ...
  return {
    ...
    reason: hasRoute && hasGate
      ...
      : 'Engineering Judgment route gate is missing; run architecture readiness again'
  };
}
...
function collectReadinessFailures(readiness) {
  ...
  if (readiness.status && readiness.status !== 'ready') {
    failures.unshift({
      id: 'readiness_status',
      status: readiness.status,
      reason: `Architecture Readiness status is ${readiness.status}`
    });
  }
  return failures;
}
...
function buildNextActions(checks, storyId) {
  ...
  if (checks.some((check) => check.id === 'engineering_judgment' && check.status !== 'pass')) {
    actions.push(`Run \`vibepro architecture readiness . --id ${storyId} --base <base-ref>\` to regenerate Engineering Judgment evidence.`);
  }
  return actions;
}
...
function requireStoryId(storyId) {
  if (!storyId) throw new Error('architecture readiness requires --id <story-id>');
  return storyId;
}
