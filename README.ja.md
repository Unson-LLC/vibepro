...
- Story intake から native Design System、modernization証跡、responsive / accessibility check、review cockpit、PR readiness までつなぐ単一の UI/UX 準備path
...
### UI/UX Modernization PR を準備する

狭いコード不具合ではなく UI/UX intent から始める場合は、この流れを使います。既存Storyから開始し、現在HEADに対して `pr prepare` がreadyになるところまでを1本の準備pathとして扱います。
...
intake prompt、外部Design System、screenshot、生成された visual hypothesis は判断材料です。実装の正本にはしません。ready判定は Story、Spec、Architecture、現行route code、VibePro-native Design System、現在の検証証跡、Agent Review証跡、Gate DAG で行います。
