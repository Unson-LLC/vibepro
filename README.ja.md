...
- merge 時の前提確認、merge 結果、branch cleanup 結果を artifact に残す `vibepro execute merge`
...
PR 作成後、checks と review が揃って merge できる状態になったら、merge も VibePro 経由で実行します。
...
監査可能性が必要な運用では、通常経路として直接 `gh pr merge` を使わないでください。`execute merge` は merge 時点の readiness、merge commit SHA、branch cleanup の成否を `.vibepro/` に記録します。
