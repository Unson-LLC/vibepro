...
専門判断の8ノードは、成果と範囲、本人とAIの分担、変更の所属、情報共有、実行と依存、構造の単純化、実行経路への到達、次の検証の価値を扱います。既存の開発モードや採択権限を置き換えるのではなく、前提・候補・未解決事項を判断へ渡す補助です。[専門判断DAGの詳細](/ja/guide/expert-judgment-nodes)に各ノードと二巡の例があります。
