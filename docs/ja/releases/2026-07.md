...
## Agent Review freshnessを検査surface基準へ

`gate_evidence`と`release_risk`の既定を、記録済みinspection
surface／release-impact surfaceへ変更しました。対象surfaceが不変なら、別セッションの
main更新、rebase、mergeだけでは失効しません。入力欠落、hash不一致、legacy差分解決不能、
理由付き`strict_head`は引き続きfail closedです。利用者の移行操作やartifact migrationは
不要で、built-in strict-role policyの復元がrollbackになります。
