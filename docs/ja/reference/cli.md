...
`sequence` は高リスク変更の検証を `targeted_validation` -> `preflight_review` -> `code_frozen` -> `expensive_verification` -> `final_review` の順で進めます。targeted validationとfreeze後のexpensive verificationには、`vibepro verify record` で作成した `.vibepro/pr/<story-id>/verification-evidence.json` を `--evidence` で指定します。preflightには、risk planが選んだroleのclosedかつpassしたcanonical Agent Reviewを指定します。任意のrepository内JSONは受理しません。freeze前にpreflight指摘を終端状態（`accepted`、`rejected`、`duplicate`、`deferred`、`false_positive`）へ disposition してください。各record後は `sequence status` が返すevidence producer commandと `follow_up_command` を順に実行し、変更時は `sequence invalidate` にsurface/fileを記録します。freezeしたHEAD、test fingerprint、commandのいずれかが変わると後続証跡は無効化されます。`final_review` は `--source agent_review --evidence .vibepro/reviews/<story>/<stage>/review-result-<role>.json` を指定し、同じStory・freeze済みHEADへ拘束されたpass、strongなparallel-subagent provenance、closed lifecycleをVibeProの正規review storeで確認できるresultだけを受理します。CIの `--coverage` は自己申告の証明ではなく、同じHEADにcommit済みの `.github/vibepro-ci-coverage.json` にあるworkflow・check・command・fingerprint完全一致mappingの選択です。
