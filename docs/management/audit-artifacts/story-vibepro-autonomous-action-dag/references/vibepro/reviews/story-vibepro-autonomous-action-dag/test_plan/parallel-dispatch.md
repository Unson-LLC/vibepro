...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-vibepro-autonomous-action-dag --stage test_plan` を実行し、その後 `vibepro pr prepare . --story-id story-vibepro-autonomous-action-dag --base <base-branch>` で次stageへ進む。
...
Review request:
`.vibepro/reviews/story-vibepro-autonomous-action-dag/test_plan/review-request-e2e_ux.md`
...
`vibepro review start . --id story-vibepro-autonomous-action-dag --stage test_plan --role e2e_ux --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-vibepro-autonomous-action-dag --stage test_plan --role e2e_ux --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
