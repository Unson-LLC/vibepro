...
| Story | dependency_cycle の SCC 縮約 — 69,490件の単純閉路列挙を「1つのSCC + 20の相互依存ペア」へ畳み、ratchet gate を載せられる粒度にする |
| Story ID | story-vibepro-dependency-cycle-scc-reduction |
| 診断フェーズ | pre_implementation |
| VibePro Runtime | vibepro@0.2.0-beta.2 commit=882c3df8c200 dirty=true |
| 種別 | unknown |
| 描画方式 | - |
| 適用チェック | secrets, xss, dependency-graph, code-quality, api-boundary |
| graphify nodes | 5977 |
| graphify edges | 13255 |
| 共通スキャン対象 | 9876件 |
...
|----|------------------|------|------|--------|-----------|------------|------|
| VP-ACTION-ARCH-001 | VP-ARCH-001 | responsibility split campaignをStory化する | 1件 | 0.021 (279 edges) | 16(file: 1, node: 111, edge: 277), 1(file: 1, node: 1, edge: 12) | src/session-efficiency-audit.js<br>src/cli.js<br>src/workspace.js | proposal_only / mutates_repository=false |
