...
| 項目 | 内容 |
|------|------|
| Run ID | 2026-08-03T010633Z |
| Story | gate:uiux_intake_judgment を人間向けPRサマリー表面に表示する |
| Story ID | story-vibepro-uiux-intake-gate-pr-summary-surfaces |
| 診断フェーズ | pre_implementation |
| VibePro Runtime | vibepro@0.2.0-beta.2 commit=2a54c080bd0c dirty=false |
| 種別 | unknown |
| 描画方式 | - |
| 適用チェック | secrets, xss, dependency-graph, code-quality, api-boundary |
| graphify nodes | 5878 |
| graphify edges | 13105 |
| 共通スキャン対象 | 9060件 |
...
| 責務混在候補 | 23件 (block: 0件, review: 23件, info: 0件) |
| リファクタリング機会 | 23件 |
| リファクタリングcampaign | 2件 |
| API route | 0件 |
...
- VP-FLOW-000: UI Storyなのに導線検査対象のUIコードが走査できない（Critical）
...
- before: 2026-08-02T183839Z / after: 2026-08-03T010633Z
- 改善: 0件 / 解消: 0件 / 悪化: 0件 / 新規: 0件
- 主な改善: なし
- 次の候補:
  - 責務混在runtime fileを分離する: 1ファイル / 1出現 (responsibility_split)
  - 責務混在runtime fileを分離する: 1ファイル / 1出現 (responsibility_split)
  - 責務混在runtime fileを分離する: 1ファイル / 1出現 (responsibility_split)
...
|----|------------------|------|------|--------|-----------|------------|------|
| VP-ACTION-ARCH-001 | VP-ARCH-001 | responsibility split campaignをStory化する | 1件 | 0.0213 (279 edges) | 20(file: 1, node: 112, edge: 279) | src/session-efficiency-audit.js<br>src/cli.js<br>src/workspace.js | proposal_only / mutates_repository=false |
