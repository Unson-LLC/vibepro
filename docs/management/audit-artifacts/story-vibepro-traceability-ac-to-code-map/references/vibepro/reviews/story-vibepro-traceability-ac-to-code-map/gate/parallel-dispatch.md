...
- Story: story-vibepro-traceability-ac-to-code-map
- Stage: gate
- Mode: policy-aware parallel review dispatch
- Required subagents: 1
- Current head: 65738829b6876c9583de19b50c7ea1d1a9b87def
- User dirty: false
- Raw dirty: false
...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-vibepro-traceability-ac-to-code-map --stage gate` を実行し、その後 `vibepro pr prepare . --story-id story-vibepro-traceability-ac-to-code-map --base <base-branch>` で次stageへ進む。
...
Review request:
`.vibepro/reviews/story-vibepro-traceability-ac-to-code-map/gate/review-request-gate_evidence.md`
...
`vibepro review record . --id story-vibepro-traceability-ac-to-code-map --stage gate --role gate_evidence --status <pass|needs_changes|block> --summary "<summary>" --inspection-summary "<inspection-summary>" --inspection-evidence <inspection-evidence> --inspection-input <ref> --judgment-delta "<initial judgment -> final judgment because evidence>" --agent-system <codex|claude_code> --execution-mode parallel_subagent --agent-id "<subagent-id>" --agent-model "<model>" --agent-reasoning-effort "<reasoning-effort>" --agent-cost-tier "<cost-tier>" --agent-transcript <artifact> --agent-closed`

Lifecycle start command:
`vibepro review start . --id story-vibepro-traceability-ac-to-code-map --stage gate --role gate_evidence --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-vibepro-traceability-ac-to-code-map --stage gate --role gate_evidence --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
