...
- status: stale
- evidence_key: evk_cbb2f20d080bd8dd47494166fd5d3b39
- first_input: false
...
Stale reasons:
- risk_surface_fingerprint: risk_surface_fingerprint changed previous=sha256:868739deafca832a454dfc5a6dbd28361e9d3e1bc6d35b6ed45fd33db10049b6 current=sha256:f8258d515f00b7ed2263958321dc874d30c09ca5a827bf077f538c734f0e0e72
...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-vibepro-design-input-judgment --stage preview` を実行し、その後 `vibepro pr prepare . --story-id story-vibepro-design-input-judgment --base <base-branch>` で次stageへ進む。
...
Review request:
`.vibepro/reviews/story-vibepro-design-input-judgment/preview/review-request-network_runtime.md`
...
subagentの結果受領後に記録するcommand:
`vibepro review record . --id story-vibepro-design-input-judgment --stage preview --role human_usability --status <pass|needs_changes|block> --summary "<summary>" --inspection-summary "<inspection-summary>" --inspection-evidence <inspection-evidence> --inspection-input <ref> --judgment-delta "<initial judgment -> final judgment because evidence>" --agent-system <codex|claude_code> --execution-mode parallel_subagent --agent-id "<subagent-id>" --agent-model "<model>" --agent-reasoning-effort "<reasoning-effort>" --agent-cost-tier "<cost-tier>" --agent-transcript <artifact> --agent-closed`

Lifecycle start command:
`vibepro review start . --id story-vibepro-design-input-judgment --stage preview --role human_usability --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-vibepro-design-input-judgment --stage preview --role human_usability --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
