...
- status: stale
- evidence_key: evk_3e61a34ff62131783217547900a599e6
- first_input: false
...
Stale reasons:
- risk_surface_fingerprint: risk_surface_fingerprint changed previous=sha256:632edddd57a1b83ccea272c9f803926405ba77c8e93a8b68a65172b6deb1e416 current=sha256:375afa5b696b82823d36e803a2c748a746797970286e3d42c581468794aaa10b
...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-vibepro-design-input-judgment --stage test_plan` を実行し、その後 `vibepro pr prepare . --story-id story-vibepro-design-input-judgment --base <base-branch>` で次stageへ進む。
...
`vibepro review record . --id story-vibepro-design-input-judgment --stage test_plan --role e2e_ux --status <pass|needs_changes|block> --summary "<summary>" --inspection-summary "<inspection-summary>" --inspection-evidence <inspection-evidence> --inspection-input <ref> --judgment-delta "<initial judgment -> final judgment because evidence>" --agent-system <codex|claude_code> --execution-mode parallel_subagent --agent-id "<subagent-id>" --agent-model "<model>" --agent-reasoning-effort "<reasoning-effort>" --agent-cost-tier "<cost-tier>" --agent-transcript <artifact> --agent-closed`

Lifecycle start command:
`vibepro review start . --id story-vibepro-design-input-judgment --stage test_plan --role e2e_ux --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-vibepro-design-input-judgment --stage test_plan --role e2e_ux --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
...
subagentの結果受領後に記録するcommand:
`vibepro review record . --id story-vibepro-design-input-judgment --stage test_plan --role gate_coverage --status <pass|needs_changes|block> --summary "<summary>" --inspection-summary "<inspection-summary>" --inspection-evidence <inspection-evidence> --inspection-input <ref> --judgment-delta "<initial judgment -> final judgment because evidence>" --agent-system <codex|claude_code> --execution-mode parallel_subagent --agent-id "<subagent-id>" --agent-model "<model>" --agent-reasoning-effort "<reasoning-effort>" --agent-cost-tier "<cost-tier>" --agent-transcript <artifact> --agent-closed`

Lifecycle start command:
`vibepro review start . --id story-vibepro-design-input-judgment --stage test_plan --role gate_coverage --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-vibepro-design-input-judgment --stage test_plan --role gate_coverage --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
