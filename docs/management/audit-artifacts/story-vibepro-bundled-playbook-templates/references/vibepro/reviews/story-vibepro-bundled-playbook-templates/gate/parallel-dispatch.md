...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-vibepro-bundled-playbook-templates --stage gate` を実行し、その後 `vibepro pr prepare . --story-id story-vibepro-bundled-playbook-templates --base <base-branch>` で次stageへ進む。
...
Review request:
`.vibepro/reviews/story-vibepro-bundled-playbook-templates/gate/review-request-gate_evidence.md`
