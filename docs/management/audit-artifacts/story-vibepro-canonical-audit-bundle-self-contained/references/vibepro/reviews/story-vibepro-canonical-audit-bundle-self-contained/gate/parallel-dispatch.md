...
- Story: story-vibepro-canonical-audit-bundle-self-contained
- Stage: gate
- Mode: policy-aware parallel review dispatch
- Required subagents: 1
- Current head: 982c9881137b4761d6dc8d117e69507e640a81cc
- User dirty: false
- Raw dirty: false
...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-vibepro-canonical-audit-bundle-self-contained --stage gate` を実行し、その後 `vibepro pr prepare . --story-id story-vibepro-canonical-audit-bundle-self-contained --base <base-branch>` で次stageへ進む。
...
Review request:
`.vibepro/reviews/story-vibepro-canonical-audit-bundle-self-contained/gate/review-request-gate_evidence.md`
...
`vibepro review record . --id story-vibepro-canonical-audit-bundle-self-contained --stage gate --role gate_evidence --status <pass|needs_changes|block> --summary "<summary>" --inspection-summary "<inspection-summary>" --inspection-evidence <inspection-evidence> --inspection-input <ref> --judgment-delta "<initial judgment -> final judgment because evidence>" --agent-system <codex|claude_code> --execution-mode parallel_subagent --agent-id "<subagent-id>" --agent-model "<model>" --agent-reasoning-effort "<reasoning-effort>" --agent-cost-tier "<cost-tier>" --agent-transcript <artifact> --agent-closed`

Lifecycle start command:
`vibepro review start . --id story-vibepro-canonical-audit-bundle-self-contained --stage gate --role gate_evidence --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-vibepro-canonical-audit-bundle-self-contained --stage gate --role gate_evidence --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
