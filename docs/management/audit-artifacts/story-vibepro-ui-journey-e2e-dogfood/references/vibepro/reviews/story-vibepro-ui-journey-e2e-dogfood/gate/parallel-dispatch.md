...
- status: stale
- evidence_key: evk_9998e0d582d992917e6733e5d20b1cd0
- first_input: false
...
Stale reasons:
- risk_surface_fingerprint: risk_surface_fingerprint changed previous=sha256:ea8b8e6eae5735b13ad5cfdd942ff9c7e027fec90641c54f5e9f74e0ff12d64f current=sha256:937fead1cc0aedc4c4857b603012220ad4bdd6716a71912d9ec990dacf9c8d04
...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-vibepro-ui-journey-e2e-dogfood --stage gate` を実行し、その後 `vibepro pr prepare . --story-id story-vibepro-ui-journey-e2e-dogfood --base <base-branch>` で次stageへ進む。
...
`vibepro review record . --id story-vibepro-ui-journey-e2e-dogfood --stage gate --role gate_evidence --status <pass|needs_changes|block> --summary "<summary>" --inspection-summary "<inspection-summary>" --inspection-evidence <inspection-evidence> --inspection-input <ref> --judgment-delta "<initial judgment -> final judgment because evidence>" --agent-system <codex|claude_code> --execution-mode parallel_subagent --agent-id "<subagent-id>" --agent-model "<model>" --agent-reasoning-effort "<reasoning-effort>" --agent-cost-tier "<cost-tier>" --agent-transcript <artifact> --agent-closed`

Lifecycle start command:
`vibepro review start . --id story-vibepro-ui-journey-e2e-dogfood --stage gate --role gate_evidence --agent-system <codex|claude_code> --agent-id "<subagent-id>" --timeout-ms 600000`

timeout/replacement/manual shutdown用Lifecycle close command:
`vibepro review close . --id story-vibepro-ui-journey-e2e-dogfood --stage gate --role gate_evidence --agent-id "<subagent-id>" --close-reason <completed|timeout|replaced|manual_shutdown>`
