...
| VP-FLOW-000 | 導線設計 | UI Storyなのに導線検査対象のUIコードが走査できない | Critical | 対象repoでVibeProを実行するか、flow_design.code_rootsで実UI実装パスを指定する。 |
...
| VP-FLOW-000 | unreviewed | implementation_gap | - | VP-FLOW-000 は対象リポジトリ内の公開面、API境界、または配信設計に対する実装不足候補として検出された。 |
