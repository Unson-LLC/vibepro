...
| Story | docs-only Storyの証跡depthを軽量プロファイルに落とし偽のbudget超過を除去する |
| Story ID | story-vibepro-docs-only-evidence-profile |
| 診断フェーズ | pre_implementation |
| VibePro Runtime | vibepro@0.2.0-beta.1 commit=d249181261d4 dirty=true |
| 種別 | unknown |
| 描画方式 | - |
| 適用チェック | secrets, xss, dependency-graph, code-quality, api-boundary |
| graphify nodes | 5625 |
| graphify edges | 12627 |
| 共通スキャン対象 | 8091件 |
...
- Scenario Gaps: 1
- Contradictions: 0
- Scenario Gap: src/merge-manager.js の `!gateAuthorization.allowed` 分岐が、Story/Spec/Architecture/Policyの受け入れ基準または方針で明示されているか確認が必要。
...
- VP-REQ-002: Storyに明示されていない重要シナリオ分岐がある（Medium）
...
- before: 2026-07-25T124326Z / after: 2026-07-25T124745Z
...
|----|------------------|------|------|--------|-----------|------------|------|
| VP-ACTION-ARCH-001 | VP-ARCH-001 | responsibility split campaignをStory化する | 1件 | 0.0218 (275 edges) | 16(file: 1, node: 111, edge: 275) | src/session-efficiency-audit.js<br>src/cli.js<br>src/workspace.js | proposal_only / mutates_repository=false |
