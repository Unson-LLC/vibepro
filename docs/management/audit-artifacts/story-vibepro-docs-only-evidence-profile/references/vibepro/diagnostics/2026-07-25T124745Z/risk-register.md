...
| VP-REQ-002 | 要件整合性 | Storyに明示されていない重要シナリオ分岐がある | Medium | 受け入れ基準に重要分岐を追加するか、既存ポリシーへの参照をStoryに残す。 |
...
| VP-REQ-002 | unreviewed | implementation_gap | - | VP-REQ-002 は対象リポジトリ内の公開面、API境界、または配信設計に対する実装不足候補として検出された。 |
