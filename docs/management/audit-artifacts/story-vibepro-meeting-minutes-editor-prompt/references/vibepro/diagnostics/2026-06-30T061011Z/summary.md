...
| Story | 見本準拠の議事録編集プロンプトを同梱skillとして提供する |
| Story ID | story-vibepro-meeting-minutes-editor-prompt |
| VibePro Runtime | vibepro@0.1.0-beta.0 commit=c8254d7184de dirty=false |
| 種別 | unknown |
| 描画方式 | - |
...
| 責務混在候補 | 20件 (block: 0件, review: 20件, info: 0件) |
| リファクタリング機会 | 20件 |
| リファクタリングcampaign | 2件 |
| API route | 0件 |
...
|----|------------------|------|------|--------|-----------|------------|------|
| VP-ACTION-ARCH-001 | VP-ARCH-001 | responsibility split campaignをStory化する | 1件 | 0.0199 (154 edges) | 14(file: 1, node: 68, edge: 154) | src/session-efficiency-audit.js<br>src/cli.js<br>src/workspace.js | proposal_only / mutates_repository=false |
...
- 優先度: medium
- 理由: VP-CAMPAIGN-REF-001 は 1件の機会を束ねるStory候補。最初に VP-OPP-ARCH-017 を確認する。
...
修正前ブリーフィング:
- リファクタリング機会: VP-OPP-ARCH-017 / responsibility_split
