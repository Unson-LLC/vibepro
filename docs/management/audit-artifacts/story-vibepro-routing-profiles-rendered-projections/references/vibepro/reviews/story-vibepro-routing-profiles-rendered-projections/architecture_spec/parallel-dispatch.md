...
8. 他のAgent Review stageを同じbatchでdispatchしない。`vibepro review status . --id story-vibepro-routing-profiles-rendered-projections --stage architecture_spec` を実行し、その後 `vibepro pr prepare . --story-id story-vibepro-routing-profiles-rendered-projections --base <base-branch>` で次stageへ進む。
