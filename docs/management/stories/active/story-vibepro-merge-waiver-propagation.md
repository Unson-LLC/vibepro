...
- 既定のreview budget policyやAgent Runtime dispatch契約の変更（ただし正規 `review close → review record` の結果回収欠落は、このStoryをVibePro自身でmergeするために必要な限定的lifecycle整合修正として含める。必須3+3役と一度のbounded repairを完遂するStory-local上限9は、2026-07-23の明示承認に基づく実行監査設定であり製品契約には含めない）
