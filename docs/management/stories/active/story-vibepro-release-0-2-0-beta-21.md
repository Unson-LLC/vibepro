...
reason: Issue #518の修正はmainへ統合済みだが、公開済みbeta.20には含まれない。既存のpost-merge release経路を使い、実装を変更せずversion metadataだけをbeta.21へ更新する。公開失敗時はdist-tagを変更せず、同一SHAのregistry、GitHub Release、fresh installが揃うまで完了としない。
---
