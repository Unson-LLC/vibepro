...
- [x] `gate:common_judgment_spine` にsub-check配列が出力される
- [x] Storyの目的またはAcceptance Criteriaを取得できない場合、`intent` は `needs_story` になる
- [x] 差分対象の現状確認証跡がない場合、`current_reality` は `needs_evidence` になる
- [x] Spec/Architecture/既存テストから不変条件が抽出できない高リスク変更では、`invariants` が `needs_evidence` になる
- [x] public API、DB、auth、agent workflow、runtime surfaceを触る変更で境界証跡がない場合、`boundaries` が `needs_evidence` になる
- [x] failure modeまたはdone evidenceが空のまま高リスク変更がreadyにならない
- [x] PR本文のEngineering Judgment説明は、route名だけでなく各sub-checkの根拠と不足を表示する
- [x] light/docs-only変更では必要最小限のspine checkに縮退し、過剰にblockしない
