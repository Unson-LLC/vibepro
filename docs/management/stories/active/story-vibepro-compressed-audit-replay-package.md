...
- current_reality: 現在の canonical audit は raw artifact と判断要約が混ざり、usage report や価値監査が通常時にも大きい JSON を読みに行きやすい。
- failure_modes: bundle hash不一致、schema不一致、展開失敗、Story不一致、missing source artifact を pass と誤認すると、handoff が fake-green になる。
- done_evidence: `test/canonical-audit-self-contained.test.js` は compact promotion、fresh checkout replay、破損bundle blocked を artifact_replay として検証する。
- done_evidence: `test/traceability-usage-report.test.js` は通常reportが compressed bundle を展開せず metadata だけでコストと replay status を読めることを検証する。
- authoritative_signal_source: replay readiness の正本は `audit-index.json` の bundle metadata と hash であり、`decision-summary.md` は人間向け入口に限定する。
