...
The Environment Graph (story-vibepro-environment-graph) now models what deploys where. This story uses it: when a change is risk-bearing *and* the topology has real deploy targets, VibePro requires that deploy/verification intent is closed as evidence before treating the PR as ready.
