...
`gate:visual_qa` の第一級証跡である residual analysis（`.vibepro/qa/<qa-id>/visual-residual.json` + `residual-analysis.md`）は、現状 Percy / Chromatic 等の外部ツールで差分を計算し JSON を手で配置する前提になっている。VibePro 自身は差分を計算しない。Playwright の組込みスクリーンショット比較を使い、baseline と current の視覚差分をローカルで計測して gate が受理する residual artifacts を直接生成できるようにする。
...
- 差分計算は Playwright 組込みの画像比較（pixelmatch 系）を利用し、新規の外部サービス依存を追加しない。
...
- [ ] VRL-S-4: `--update-baseline` 実行後の再実行では当該 probe の差分が 0 近傍になる。
