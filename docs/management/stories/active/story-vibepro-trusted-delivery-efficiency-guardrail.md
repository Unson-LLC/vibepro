...
- [ ] TDEG-S-1: Story/Run policyは時間、token/cost、subagent、role別Review、repair batch、expensive verificationのbudgetをmachine-readableに持ち、未指定値と未計測値を0へ変換しない。例外的な移行コストは理由付きStory overrideで局所化し、通常Storyの既定上限を緩めない。
- [ ] TDEG-S-2: subagent spawn前のauthorizationは、意図するmodel、閉じるrisk、期待するjudgment delta、再利用可能なevidence、Story全体の実行済みReviewと未消費予約を含む残予算を原子的に記録・評価する。host coordinatorは`dispatch`以外ではproviderを起動せず、spawn後の`review start`は同一HEAD/surface/modelへbindされた未消費authorizationを必須とする。VibeProはprovider固有のspawn APIやcompletion notification配送を所有せず、逸脱したhost spawnを事後に正当化しない。
- [ ] TDEG-S-3: source、Spec、test、review surfaceがfreezeされていない状態ではfinal Reviewを開始せず、preflightとfinal Reviewを区別する。
- [ ] TDEG-S-4: 同一Story、stage、role、surface digest、HEADのdispatch authorizationはidempotentで、running、completed pass、未回収result、未消費予約の重複起動を防ぎ、並列coordinatorもStory budgetをoverbookできない。
...
- [ ] TDEG-S-13: Review roleは実変更surfaceから選ばれ、`workflow_heavy`だけを理由にUI/network roleを要求しない。repoがRisk-adaptive Validation Sequenceによるworkflow state transition orderingの所有を明示設定した場合だけ、同じriskを再確認する5つのcheckpoint roleは重複dispatchせず、未設定repoでは既存checkpoint契約を維持する。どちらの場合も独立final gate/release reviewは維持する。
