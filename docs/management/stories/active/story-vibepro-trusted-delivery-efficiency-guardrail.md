...
- [ ] TDEG-S-2: subagent spawn前のauthorizationは、意図するmodel、閉じるrisk、期待するjudgment delta、再利用可能なevidence、Story全体の実行済みReviewと未消費予約を含む残予算を原子的に記録・評価し、`dispatch`以外では起動しない。spawn後の`review start`は同一HEAD/surface/modelへbindされた未消費authorizationを必須とする。
