...
- [ ] TDEG-S-13: Review roleは実変更surfaceから選ばれ、`workflow_heavy`だけを理由にUI/network roleを要求しない。Risk-adaptive Validation Sequenceがcheckpoint orderingを所有する場合、同じriskを再確認する5つのcheckpoint roleは重複dispatchせず、独立final gate/release reviewは維持する。
