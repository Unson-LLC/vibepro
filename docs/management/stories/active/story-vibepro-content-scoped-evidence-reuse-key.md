...
reason: "review freshness判定は#285でcontent-scoped化済みだが、evidence reuse keyがhead_shaと検証timestampを含むため修正コミットごとに全ロールのreuseがmissし、全パネル再レビューが発火する。key構成の変更に限定し、`freshness_mode: strict_head`が設定されたロール（ロール名のハードコードではなく`.vibepro/config.json` `agent_reviews.roles.<role>.freshness_mode` + `freshness_reason`で個別設定され、`role.freshness_policy.effective_mode`で汎用判定される）の保守的な再レビュー方針は変えない。`gate_evidence` / `release_risk` は現状デフォルトの`content_surface`で運用されており（strict_head固定ロールではない）、その扱いも本Storyの前後で変わらない。rollback: key構成をrevertすれば従来のhead束縛reuseへ戻る。"
...
- 訂正（2026-08-05）: 当初CRK-S-3と`reason`は`gate_evidence` / `release_risk`を「built-in strict HEAD role」と記載していたが、実際にはロール名のハードコードではなく、`.vibepro/config.json` `agent_reviews.roles.<role>.freshness_mode`（+`freshness_reason`）で個別設定される汎用機構であり、両ロールとも現状はデフォルトの`content_surface`で運用されている（strict_head固定ではない）。Gate Evidence Agent Reviewの指摘を受け事実に合わせて記述を訂正した。意図（`strict_head`設定ロールの保守的な再レビュー方針は本Storyで変えない）は変更していない。
...
- [ ] CRK-S-3: `freshness_mode: strict_head`が設定されたロール（ロール名のハードコードではなく、`.vibepro/config.json` `agent_reviews.roles.<role>.freshness_mode: strict_head` + `freshness_reason`で個別設定され、`role.freshness_policy.effective_mode`で汎用判定される）は従来どおりhead変更で再レビューされ、本Storyの対象外として挙動が変わらない。`gate_evidence` / `release_risk` は現状デフォルトの`content_surface`で運用されており（strict_head固定ロールではない）、この扱いも本Storyでは変わらない。
