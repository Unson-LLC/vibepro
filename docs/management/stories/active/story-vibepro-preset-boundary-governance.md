...
source:
  ...
  id: VP-SELF-004
  title: "example dialog app診断にexample travel app由来の語彙が残る疑い"
architecture_docs:
...
**So that** example dialog appのような別プロダクトでも、対象固有の文脈から正しくStoryとArchitectureを立ち上げられる

## Background

example dialog appの診断では、診断だけなら問題が見えにくい一方で、generic化が完全でなければexample travel appやホテル業務など過去プロジェクトの情報が混入する懸念があった。
...
- [ ] default / generic presetに、example travel app、ホテル、shadow-callなど特定プロジェクト固有語彙が含まれない
- [ ] project presetは明示選択された場合だけ有効になる
- [ ] Story生成、diagnosis、architecture suggestionの各出力に anti-leak fixture がある
- [ ] example dialog app、example outreach app、空のNext.jsアプリなど異なる文脈で、対象外語彙が出ない回帰テストがある
