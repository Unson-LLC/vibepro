...
- `PCR-CON-005`: alphaは `alpha`、betaは `beta` と `latest`、stableは `latest` を設定し、各tagはregistryで可視な対象channelのversionとmutation前の現行tagを下限に最大SemVerへ単調に収束する。npm公開とGitHub Releaseの不可逆区間はpackage単位のatomic git-ref leaseで直列化し、古いrunのread-write interleaveによるtag巻き戻しを防ぐ。
