...
- `SAB-CONTRACT-007`: unreadable JSONL or any malformed selected row fails attribution closed as unavailable with a bounded reason.

# Verification

The synthetic fixtures assert primary and upper-bound counts, accounting non-regression,
exhaustive categories, threshold risk, explicit unavailable/read/parse-failure output,
