...
- **S-004**: refresh結果は `policy_sync` フィールド（status: `synced` / `unchanged` / `skipped` / `failed` + `sections_updated`）を持ち、execution state経由で監査できる。親configの欠如・破損はrefresh全体を失敗させない（fail-soft）。
...
- `src/managed-worktree.js` — `syncWorktreePolicySections()` / `POLICY_CONFIG_SECTIONS` / `policy_sync`
- `test/managed-worktree-policy-resync.test.js` — contract tests（5件）
