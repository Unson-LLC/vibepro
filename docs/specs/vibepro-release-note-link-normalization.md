...
- `RNLN-001`: release section内のinline Markdown destinationが`docs/`で始まる場合、通常リンクをcanonical GitHub blob URL、画像をraw URLへ変換する。angle-wrapped destinationはwrapperを除去し、既存の有効なpercent escapeを保持したまま空白をpercent encodeする。reference-style definitionはlink/image用途を一意に判定できないため変換しない。
