...
built modeはrepository内の実directoryだけを受け入れ、symlinkを含むrepository外逸脱を拒否する。配下のHTMLを再帰的に最大400件走査し、robots/llms/header evidenceも同じbuild rootから取得する。

### PDLT-CONTRACT-003: live境界

live modeはHTTP(S) GET、同一originのbase rootとsitemap loc、最大40ページ、1応答2 MiB、1要求10秒に限定する。redirect、外部origin、非HTTP(S)、過大応答、取得失敗は検査せずerror evidenceへ残す。

### PDLT-CONTRACT-004: coverage honesty

`scan_coverage` はmode、roots、発見数、検査数、失敗数、errors、limits、status、reasonを持つ。検査数0は `inconclusive` であり、項目別finding 0件をpassの証拠にしない。1件以上検査できたcoverageだけがpassとなる。

### PDLT-CONTRACT-005: findings優先

coverageと品質findingを独立に扱う。block/review findingはtop-levelの `fail / needs_review` としてcoverage判定より優先し、既存finding ID、severity、suppressionを変更しない。
...
`test/public-discovery-live-targets.test.js` が3mode、優先順位、同一origin、上限、明示エラー、0件inconclusive、findings優先、CLI/artifact表示を検証する。既存Public Discovery回帰テストでsource mode、route分類、metadata継承、suppression互換を検証する。
