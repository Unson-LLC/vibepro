...
`scan_coverage` はmode、roots、上限適用前の発見数、検査可能数、選択数、検査数、除外数と理由別集計、失敗数、errors、limits、status、reasonを持つ。検査数0は `inconclusive` であり、項目別finding 0件をpassの証拠にしない。1件以上検査できたcoverageだけがpassとなる。
