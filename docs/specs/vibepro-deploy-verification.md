...
- `AP-DV-5`: Do not add the gate node without its edges; node and edges share one conditional block so connectivity cannot break.

## Verification

- `V-DV-1`: A test asserts the gate is `needs_evidence` (blocking) for a workflow-heavy change with vercel/fly deploy targets, lists targets, has both edges wired, and flips to `passed` after a waiver.
