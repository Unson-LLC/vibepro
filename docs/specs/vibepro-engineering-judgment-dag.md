...
- `S-EJD-7`: When changed files touch secret/credential surfaces (`.env` (non-template), `*.pem`/`*.key`/`*.p12`/`*.pfx`/`*.keystore`/`*.jks`, `id_rsa`/`id_ed25519`-style private keys, `.npmrc`/`.pypirc`/`.netrc`, or `secret(s)`/`credential(s)` paths), `gate:safety_secret_surface` (`type: safety_surface_gate`) appears and is `needs_evidence` (blocking) until a `secret_exposure` decision (with `--secret-action`) or an explicit waiver against the gate source is recorded. When no secret surface is touched the gate is absent (no friction); `.env.example`/`.sample`/`.template` files do not trigger it.
...
- `V-EJD-6`: Focused PR prepare tests assert `gate:safety_secret_surface` is `needs_evidence` (blocking) for a `.env`/secret-surface change and `passed` after a `secret_exposure` decision, and is absent for non-secret changes and `.env.example` templates.
