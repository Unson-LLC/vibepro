...
- INV-001: `test/**/*.{test,spec}.{js,ts,jsx,tsx,cjs,mjs}` のうち mkdtemp / mkdtempSync / tmpdir() / os.tmpdir() を呼ぶ全ファイルは `test/support/scratch-tmpdir.js` をimportする。conformanceテスト（`test/scratch-tmpdir-conformance.test.js`）が違反ファイルを名指しして失敗する。
