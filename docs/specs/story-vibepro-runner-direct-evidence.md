...
- Every fact the runner computes is protected from agent input, including the ones that are not test counts: `timed_out`, `output_limit_exceeded`, `output_metrics`, `timeout_ms`, `max_output_bytes` and `harness_env_removed`. The declared limits are recorded whether or not they were hit.
- If anything between taking the run files and committing the record fails, the previous run's artifact and log are restored, and a restore that itself fails is reported alongside the original error rather than swallowed.
