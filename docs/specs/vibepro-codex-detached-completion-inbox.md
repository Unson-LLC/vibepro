...
- Completion callbacks persist immutable dispatch-scoped Inbox events before wakeup. The Inbox accepts only a VibePro-owned allowlist schema and rejects unknown provider payload fields before persistence. Reconcile reads the Inbox even when wakeup is lost.
