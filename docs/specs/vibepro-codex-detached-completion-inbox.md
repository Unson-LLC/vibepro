...
- Runtime CLI composition resolves host state from the explicit repository argument, not the caller shell cwd. Schema-valid bounded judgment detail is accepted consistently by worker output, partial Inbox events, and completed result judgments.
