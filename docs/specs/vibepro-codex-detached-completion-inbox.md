...
- `createCodexGuardedRunBridge.resumeFromWake` is the push delivery entrypoint for a host to resume the persisted Run, and the CLI selects this bridge when `codexSubagentHost` is injected.
...
- `createCodexGuardedRunBridge` is the production composition boundary for Inbox, adapter, coordinator, Guarded Run, and the injected host implementation.
