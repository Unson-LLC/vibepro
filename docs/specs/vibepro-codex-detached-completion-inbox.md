...
- Only checkpoints and completed partial judgments count as progress. No-progress can start one next bounded attempt for the same logical dispatch with only unfinished judgments; total wall-clock is not reset between attempts. Provider-reported cost is accumulated across recovery attempts, and wall-clock, attempt, and cost hard limits are evaluated before another spawn; an exceeded limit produces `stalled` and host containment.
- Persisted `logical_started_at` and `attempt_started_at` are independent of generic reconcile `updated_at`; a successor process cannot reset the no-progress deadline. Reconcile preserves `running_detached` while the provider remains running.
- Runtime CLI composition resolves the caller from the explicit repository argument, not the shell cwd. Every successor observation passes the persisted dispatch's managed-worktree authority to host status, shutdown, and durable completion drain, so a fresh process started from the source checkout can rediscover provider state and structured events without process-local root memory. Schema-valid bounded judgment detail is accepted consistently by worker output, partial Inbox events, and completed result judgments.
...
- The built-in host executes `codex exec` in a detached read-only worker process group with a VibePro-owned prompt/output schema, persists only sanitized state and structured partial/completion events, and attempts repo-local `runtime-ingest`; a parent subscription scan remains the fallback when direct delivery is unavailable. Host containment signals the process group so the real Codex descendant cannot outlive a bounded shutdown.
...
The contract, integration, and E2E surfaces are bound to `test/bin-entrypoint.test.js`, `test/agent-completion-inbox.test.js`, `test/codex-subagent-host.test.js`, `test/codex-subagent-runtime-adapter.test.js`, `test/guarded-run-session.test.js`, and `test/e2e/story-vibepro-codex-detached-completion-inbox-main.spec.ts`. The production-host test starts a detached CLI worker through a fake executable at the real process boundary, proves cross-process idempotency and ordered partial/completion delivery, proves process-group shutdown contains the real Codex descendant, creates a fresh host rooted at a different caller, then recovers status and events from the persisted managed authority; raw JSONL remains absent from persisted events. Negative coverage includes malformed, nested, oversize, or out-of-schema Inbox data, missing host delivery capability, provider/dispatch correlation mismatch, concurrent duplicate start, mismatched completion and partial-result surfaces, surface changes without path evidence, duplicate progress, successor-process deadline preservation, detached-state preservation, source-to-managed successor lookup, and lost wakeup. The E2E terminates the parent-side callback and completes through a separate OS process invoking `runtime-ingest`, including a non-empty structured finding, without assigning `resumeFromWake` by hand, then replays the same wake without a second Agent Review side effect. Guarded Run also proves that an existing logical dispatch can rebind to a rebased HEAD only when the caller explicitly asserts an unchanged inspection surface, accept the provider result bound to the recorded predecessor HEAD, and close the review without another spawn.
