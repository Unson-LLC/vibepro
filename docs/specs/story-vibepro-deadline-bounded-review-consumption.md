...
（clauses: INV-001 / S-001 / S-002 / INV-002 / C-001 / C-002 / S-003 / INV-003、diagrams: flow）。
...
- C-001 (DBA-S-5): `src/agent-review.js` の `authorizeAgentReviewDispatch` と `src/pr-manager.js` の
  ...
  変わらない。
- C-002 (DBA-S-6): `aggregateDeliveryMetrics` の戻り値に `deadline_excluded_ms` / `deadline_excluded_count` が
  追加され、`withDeadlineExclusionVisibility`（`src/agent-review.js`）が dispatch decision の `budget` へ、
  `story-run-portfolio.js` の `emptyCostAttribution` / `COUNT_COST_KEYS` が永続化される portfolio
  `cost_attribution` へ、それぞれ両フィールドを伝播させる。
...
| DBA-S-6 | C-002 | deadline_excluded_ms/countの戻り値追加とdispatch evidence/portfolioへの伝播 |
