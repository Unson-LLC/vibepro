...
`gate_dag.overall_status` が `ready_for_review` ではない場合、`pr create` は既定で失敗する。未完了Gateを意図的に残す場合は `--allow-needs-verification --verification-waiver <reason>` を両方指定し、waiver理由を `pr-create.json` / `pr-create.html` に監査証跡として残す。waiver理由なしの bypass は許可しない。

CLI理由付きwaiverを使う場合でも、VibeProは `gate_override.waiver_policy: "cli_reason"`、`critical_unresolved_gates[]`、`completion_quality`、`required_evidence[]`、`toolchain` を記録する。これにより、Architecture未承認、Spec未確定、E2E未達、Visual QA未達、Requirement未解決、失敗Gateが残るPRをレビュー時に見逃さない。
