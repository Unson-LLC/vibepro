...
`gate-dag.json` は、Story / Architecture / Spec / Code / Acceptance Criteria / Unit Gate / Integration Gate / E2E Gate / PR Gate をDAGとして記録する。各Gateは `status`、`required`、`command`、`reason`、必要に応じて `artifact_expectation` を持つ。E2E Gate は Playwright CLI を標準とし、検出できた `package.json` script、Playwright依存、または `npx playwright test` を候補コマンドとして出す。
...
- `story_docs`: `docs/management/stories/`
