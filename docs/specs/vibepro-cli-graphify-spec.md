...
`--preset <id>` 指定時は、Story抽出とGraph Coverageのpath判定に使うリポジトリ構造プリセットを明示する。指定がない場合は `.vibepro/config.json` の `story_catalog.preset` を使い、それもない場合は `next-app` を使う。初期プリセットは `next-app` と `modular-web` を持つ。生成される `story-catalog.json` には `source.preset` として実際に使ったプリセットを記録する。
