...
- `VibePro refactoring delta`: 前後診断のDRY/責務分離候補を `5ファイル / 8出現 -> 3ファイル / 5出現` の形で表示
...
`diagnose` 実行後、同一Storyの前回runがある場合は `evidence.refactoring_delta` と `runs[0].artifacts.refactoring_delta` を記録し、重複query形状や責務混在候補が前回から改善・解消・悪化・新規のどれかを算出する。前回runがない場合は `status: no_baseline` とする。
