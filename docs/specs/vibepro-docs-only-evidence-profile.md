...
- `DOE-CONTRACT-001a` (C-001): under `docs/`, the deployed manual surface (`docs/.vitepress/`, `docs/public/`) and machine-read responsibility contracts (`docs/contracts/`) MUST classify as `product_code`; the requirement trees (`docs/management/`, `docs/specs/`, `docs/architecture/`, `docs/adr/`) MUST classify as `docs` whatever their file extension; anything else under `docs/` MUST classify as `docs` only when it carries a documentation extension.
