...
- correctionは誤前提、訂正後前提、理由、workspace相対の代替証拠とSHA-256を保持する。
- correction後は元judgeとは異なるfresh judgeの紐づく再裁定が必要で、`judged_sound` だけが解決する。
- resolverはevent配列順ではなく明示参照を使い、不正な系譜はfail closedにする。
- `implementation_unsound`、既存のhuman judgment経路、critical gate性は従来どおり維持する。

## 検証

- Unit: `test/judgment-adjudication.test.js` の `CPR-S-001`〜`CPR-S-006`
- CLI E2E: `test/e2e/story-vibepro-classifier-premise-recovery-main.test.js` の `CPR-E2E-001`
