...
## Surface-aware Agent Review freshness

`gate_evidence` and `release_risk` now use recorded inspection and release-impact
surfaces by default. Unrelated main movement, rebases, and merges no longer
invalidate them when those surfaces are unchanged. Missing inputs, hash
mismatches, unresolved legacy deltas, and explicit reasoned `strict_head`
policies still fail closed. No user action or artifact migration is required;
restoring the built-in strict-role policy remains the rollback.
