...
`sequence` runs high-risk validation in this order: `targeted_validation` -> `preflight_review` -> `code_frozen` -> `expensive_verification` -> `final_review`. For targeted and post-freeze expensive verification, create `.vibepro/pr/<story-id>/verification-evidence.json` through `vibepro verify record` with a verified result artifact, target, scenario, strict HEAD binding, `--observed test_fingerprint=<sha>`, and `--observed validation_phase=<phase>`; arbitrary paths, missing/unrecognized artifacts, future timestamps, and pre-freeze expensive runs are rejected. Preflight instead requires a closed passing canonical Agent Review whose role is selected by the risk plan. `sequence status` returns the evidence producer command followed by `follow_up_command`. Preflight findings must use a terminal disposition (`accepted`, `rejected`, `duplicate`, `deferred`, or `false_positive`) before freeze. Changing the frozen HEAD, test fingerprint, or command invalidates downstream evidence. `final_review` requires an existing passing Agent Review JSON for the same Story and frozen HEAD, with strong parallel-subagent provenance and a closed lifecycle.
