...
既存のfinding判定は変更しない。block findingは `fail`、review findingは `needs_review` とし、常にcoverageより優先する。coverageの件数・失敗・除外証跡はfindingと独立に保持するが、`scan_coverage.status` は `pass / inconclusive / fail / needs_review` を返す。ページ検査0件はfindingの有無にかかわらず `inconclusive` とし、1件以上検査した場合もblock/review findingがあればcoverage passを残さない。top-level statusは強いfindingがなければcoverage statusを採用する。`inconclusive` は既存Storyの契約どおり非ブロッキングで、check aggregateはpassを維持しつつ `inconclusive_count` を持つ。
...
- `scan_coverage`:
  ...
  - `status`: `pass | inconclusive | fail | needs_review`
...
5. finding risk summaryを先に確定し、次に `resolveScanConclusiveness` で0件coverageを判定する。1件以上ならfinding statusをcoverage statusへ反映し、findingなしの場合だけpassにする。
...
- rootを含め検査可能ページが0件ならcoverageは `inconclusive`。findingが同時にある場合もcoverage行は `inconclusive` のまま、top-levelは強いfindingを優先する。1件以上検査できた場合はblock/review findingをcoverage行にも反映し、passとの矛盾を残さない。
