...
`task execute` does not introduce a second gate engine. It projects the checkpoint sequence into `execution.json` / `execution.md` as a progressive gate plan:

- pre-implementation: `story`, `implementation-start`, `test-plan`
- post-implementation / pre-PR: `implementation-complete`, `verification`
- final consistency: `pr prepare`, then `pr`

Agent Review dispatch guidance is attached to the checkpoint stage that owns the review stage. This makes PR preparation a final consistency check instead of the first moment where all subagent work is discovered.
