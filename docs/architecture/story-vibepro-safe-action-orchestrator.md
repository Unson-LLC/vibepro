...
Actionは`id`、`classification`、`depends_on`、`mutates_repository`、`execute`を持つ閉じたregistry entryとする。分類は`read_only`、`repo_local_safe`、`approval_required`、`forbidden`のみ。最初の実行planは次の2 nodeに限定する。
