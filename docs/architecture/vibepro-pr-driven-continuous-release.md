...
`pull_request_target.closed`（`merged == true`、base=`main`）を単一の開始点とする。fork由来PRでもdefault branch上の信頼済みworkflow定義へ書込み権限とrepository secretsを渡し、PRのコードは実行せず、マージ済みSHAだけを不変なpackage build対象としてcheckoutする。マージ後にLLMは呼ばず、VibeProがPR作成時に生成した `Release Notes` 契約を決定的なNode scriptで抽出する。抽出結果は同一PR番号のmarkerで月次リリース履歴と `CHANGELOG.md` にupsertし、bot commitを `main` へpushする。
...
- Delivery: `.github/workflows/post-merge-release.yml` はdefault branchの信頼済み定義とprojection codeで開始し、当該merge commitだけをdetached checkoutして依存もそのSHAで再構築し、npm、GitHub Releaseの順に実行する。成功後に最新mainへ戻り、docs commitとCloudflare deployを行う。各段と再実行手順は成功・失敗を問わずActions summaryへ追記する。
