...
このPortfolio互換修正はDAG本体と独立してrevertでき、永続stateやschemaのmigrationを伴わない。再試行が重複開始を生む、lock ownershipの検証を弱める、または新たな競合回帰を示す証拠が出た場合は修正だけを戻し、解放済み競合を従来のtyped stopへ戻す。
