...
The gate is wired `gate:pr_route_classification -> gate:deploy_verification -> gate:pr_body_contract`. Because `gate:dag_connectivity` requires every node to be reachable from `story` and to reach `pr`, a node added without these edges would (correctly) fail connectivity; both edges are added in the same conditional block as the node so they cannot drift apart.
