# Architecture: VibePro 0.2.0-beta.6 release

## 決定

PR #450を含む現在のmainから、version metadataだけを`0.2.0-beta.6`へ上げる。
version増加を検出した`post-merge-release.yml`が、PRのmerge commitをcheckoutして
typecheck、全テスト、npm pack dry-runを行い、その同じcommitからnpm publishと
...
beta.5以降にmainへmerge済みの変更が含まれ、Issue #449の成果物整合性修正をbeta利用者へ届ける。
