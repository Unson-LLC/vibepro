...
- Cross-Story learning is an explicit source/consumer artifact reference with SHA-256 digest and reason. Raw transcript and session paths fail closed.
...
Portfolio state is restart-safe because every operation reloads and validates the canonical artifact. A stopped child remains selected until a typed decision is persisted. Scope contamination throws `scope_contamination` before state advancement. The operator can export the ordered Story ids and continue individual child Runs if this additive controller is rolled back.
