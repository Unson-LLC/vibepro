...
- E2E: repo-local CLIのhost module解決とruntime command、Codex host harnessのspawn→600000ms監視境界→detached継続→親process callback破棄→後継`runtime-ingest`→Inbox→closed review bridgeを再生し、別テストで同一process pushと通知喪失時のreconcile fallbackを証明する。
