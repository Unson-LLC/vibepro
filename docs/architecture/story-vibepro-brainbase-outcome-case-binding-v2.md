...
成果ケース契約はv2 contextの `outcome_case`、Storyの `outcome_case`、PR準備の `outcome_case` に投影する。既存Storyがない一般bindは書込み前に失敗し、任意の `storyDeclaration` も拒否する。公開されたStory追加契約は正規化済みCLI項目だけを受け、Story重複検査とtraceability作成を含む完全な `story add` 処理を行う。その同じモジュール内でだけ非exportの宣言capabilityを使い、署名済みhandoffを検証してStory宣言を取引に含める。したがって一般bind APIも公開importも任意または不完全なStory宣言を渡せない。handoff sourceと消費ledgerの `source_artifact` はcanonicalなrepo相対パスに限定し、実パスを解決してrepo root内に留まることを入力、ledger書込み、journal回復、PR読戻しの各境界で検証する。絶対パス、正規化で `..` となる表現、percent-encoded traversal、Windows区切りによるtraversal、repo外へ向くシンボリックリンクは書込み前にfail closedする。managed bindはconfig、Context、bind receipt、消費ledgerを1つの耐久ジャーナルへ記録し、全ファイルの置換後にcommit markerを書き込む。中断時はmarkerのない部分投影をPR準備が信頼せず、次のbindが同じジャーナルを前進回復するため、`bound` を返さない。

PR準備は、Context・Story・receipt・markerの局所一致だけを信頼しない。receiptに保持した署名済みmanaged handoff v2を設定済み信頼鍵で再検証し、ledgerのcanonical `source_artifact` をrepo root内から再読込してreceipt digestも検証する。そのうえでcanonical `outcome_case`、receipt/context digest、Story ID、project、repository、repository root、base SHA、resolution/turn、ledgerを照合できた場合だけStory値を投影する。ローカルの `signature_trusted` のような自己申告フラグは権威にしない。v1 ContextだけのStoryは成果ケース未連携であり、`outcome_case_status: none` / `not_linked` と表示して復旧判断を付けない。v2を示す投影を信頼できない場合は、改ざん・期限切れを `untrusted`、marker欠落や不足値を `partial`、読出し不能を `unknown` と安全なreason code・再bind/復旧判断をPR準備JSONとPR本文へ表示する。`integration status`、`doctor`、Story診断レポートはこの再検証を行わず、成果ケース信頼性を `not_evaluated` と表示して `pr prepare` へ誘導する。PR準備の `technical_completion` は信頼済み検証証跡の有無を明示するが、受入条件と証跡の対応が不明な場合は `technical_complete: false` と `status: unknown` を返す。
