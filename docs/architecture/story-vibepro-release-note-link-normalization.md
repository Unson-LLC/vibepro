...
VitePress rendererはCLI command判定後、Markdown投影を行う`project`と`release-body`でだけ初期化する。`plan`と`publish-npm`はparser初期化に依存せず、PR番号markerによるupsertとnpm公開ロジックにも変更を持ち込まない。canonical URLはGitHub providerと既定branchの可用性に依存するが、projector自身はnetwork fetchを行わない。VitePress buildは構文/dead-link gateであり外部provider到達性の保証ではないため、provider障害または既定branch変更はpost-merge workflow停止後にroot定数を修正し再投影する運用riskとして明示する。

運用上の観測点はpost-merge workflowのproject stepとVitePress buildで、失敗時はrelease note commit/deployが停止する。rollbackは正規化commitをrevertし、対象PR本文をabsolute URLへ直してから旧projectorで再投影する。生成済みrelease pageをbranch差分へ含めることはrollback前提にしない。永続schema migrationやversion skewはない。
