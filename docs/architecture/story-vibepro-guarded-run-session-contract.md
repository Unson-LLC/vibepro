...
- `--repair-linked-copy` on every `execute` subcommand other than `watch` returns `repair_linked_copy_not_supported` before Story lookup, Git identity resolution, legacy/Run reads, or mutation. The option is never a silent no-op.
