...
`execute run` holds an exclusive Story-scoped creation lock from the initial legacy-state absence check through `startExecution` and Run authority commit. Every linked worktree derives that lock from Git's canonical common directory and stores it beneath that directory as the repository-unique namespace, both before and after legacy authority exists. It never derives repository identity from the common directory's parent: conventional `.git`, `--separate-git-dir`, and bare-backed layouts therefore share within one repository without allowing sibling repositories to collide. This lock serializes Run creators only; it does not claim to lock or alter legacy `execute start`. A competing invocation returns typed `run_creation_locked` without mutation. The lock is removed in `finally`; an existing orphan lock remains fail-closed and is reported with its artifact rather than silently stolen.
