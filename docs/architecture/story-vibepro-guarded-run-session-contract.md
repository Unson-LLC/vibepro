...
For `watch`, `resume`, and `cancel`, omitted `--run-id` selects the newest Run by `created_at` descending and then `run_id` lexical descending. No candidates fail with `run_not_found`. `execute status` is deliberately excluded: omitted `--run-id` always follows the exact legacy route, while Run status requires an explicit valid `--run-id`.
