...
Target architecture conformance is compared to latest `origin/main` at `d08f42d88a99624bcff4efd8520e55ce2aee8748`: 69 baseline violations (66 undeclared dependencies, 2 budget violations, 1 orphan). New code belongs to `run-session`, adds no run-session-to-CLI dependency, and must not increase the total.
