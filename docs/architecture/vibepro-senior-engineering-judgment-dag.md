...
問題設定が妥当な場合だけ、採用済み履歴、観測済みの外部成果、現在制約の確度から一つの開発モードを選ぶ。
外部成果が不変または悪化した構造加算が境界以後にあれば `SIMPLIFY`、成果または現在の制約が未確認なら
`VALIDATE`、確認済み制約があり前二条件に該当しなければ `VALUE` とする。固定回数、Story数、検討中バッチの
ラベルでは分岐しない。
...
10. 同じ履歴、外部成果、現在制約からは、検討中バッチのメタデータに関係なく同じ開発モードを選ぶ。
