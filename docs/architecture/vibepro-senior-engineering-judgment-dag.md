...
共通スパインは、入力の健全性、ゴール、違和感、問題設定、判断コストの順で必ず進む。問題設定が妥当な
場合だけ必要な判断軸を展開し、仮説・予測・証拠・反証を枝単位で評価する。到達した枝だけをfan-inし、
制約による選択肢の剪定を経て、人間とCIへ推薦を渡す。
...
確認されたリスクと不変条件を突き合わせ、制約違反の選択肢を除外する。好みやコストは比較材料にするが、
不変条件と同じ強さでは扱わない。
...
ゴール整合性と問題設定は軸ではなく、全軸より前の共通スパインである。
