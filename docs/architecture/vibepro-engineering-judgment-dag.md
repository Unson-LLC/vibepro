...
The `agent_workflow` route adds the second enforced gate, `gate:judgment_agent_workflow_evidence_lifecycle` (`type: agent_evidence_lifecycle_gate`). This is the **route axis**, not the risk axis: `gate:agent_review` already scales staged reviews by risk profile, but a change to agent/gate/dag/skill/mcp machinery can classify as low risk and still ship with zero agent-review evidence. The lifecycle gate closes that gap. It reuses the existing `agent_reviews` lifecycle data (`required_review_count`, `unmet_required_review_count`, `stale_result_count`, `lifecycle_timed_out_count`, `block_result_count`) and is `passed` only when that lifecycle is closed for the current git state (or a waiver is recorded against the gate source). The other agent_workflow judgment gates (`context_acquisition`, `tool_boundary`, `delegation_policy`, `human_decision_contract`) stay advisory. `delegation_policy` was evaluated as the next enforcement candidate but rejected as redundant: `gate:agent_review` (risk axis) already enforces parallel-subagent provenance, lifecycle closure, and required-stage coverage, and `gate:judgment_agent_workflow_evidence_lifecycle` (route axis) already enforces closure for agent changes — so a delegation-plan gate would never fire independently.

A third enforced gate, `gate:safety_secret_surface` (`type: safety_surface_gate`), is route-independent and surface-driven, mapping to the AGENTS.MD safety axis ("secrets handled by procedure, not goodwill"). It appears only when changed files touch secret/credential surfaces (non-template `.env`, private keys, keystores, `.npmrc`/`.pypirc`/`.netrc`, `secret(s)`/`credential(s)` paths). When present it is `needs_evidence` until a `secret_exposure` decision or a waiver is recorded against the gate source; otherwise it is absent, so non-secret changes carry no friction. This is proactive — the change touching a secret surface triggers it — distinct from `gate:decision_record`, which only ensures already-recorded decisions are closed.
