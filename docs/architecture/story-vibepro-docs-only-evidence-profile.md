...
- Not everything under `docs/` is documentation. This repository builds and deploys a public manual from `docs/`, so `docs/.vitepress/` (site config), `docs/public/` (the deployed static surface, including security headers and redirects), and `docs/contracts/` (machine-read responsibility contracts that drive gate behaviour) are `product_code`. The requirement trees `docs/management/`, `docs/specs/`, `docs/architecture/`, and `docs/adr/` are documentation whatever their file extension, because a Spec is routinely serialized as JSON. Anything else under `docs/` is documentation only if it carries a documentation extension.
- Machine-read registries that live inside the documentation trees are `product_code`, because VibePro reads them at runtime to drive verdicts: `docs/architecture/target-model.json` (architecture conformance and senior-gap ideal state), `docs/management/roi-ledger/` (the central gate outcome ledger), and `docs/management/responsibility-authority/`.
- Twelve paths under `.vibepro/` are tracked today (`git ls-files .vibepro`): `config.json`, one Spec, and ten task-plan artifacts. All classify as `evidence_artifacts`, and `config.json` in particular carries review-role policy and evidence budgets alongside the Story catalog that a documentation Story must register itself in. Treating them as `evidence_artifacts` is what makes the docs-only case reachable at all; the accepted, bounded consequence is that a policy edit bundled with a documentation edit can still read as docs-only. The consequence is confined to depth and budget scope — no gate requirement reads this verdict — and every tolerated record is named in `sample_evidence_artifact_paths`, so the verdict is never silent about what it accepted.
...
- What it adds is attribution: `docs_only_change`, `planner_inputs.docs_only_status`, `planner_inputs.docs_only_reason`, and `default_depth_reason`. Risk surfaces still escalate for a docs-only change — `targeted_full_surfaces` and `risk_signals` are computed identically — and documentation never weakens gate or review requirements.
...
- Switching the ratio verdict off must not also drop the ratio-derived **floor**. `normal` and `high` grant an effective line budget of `max(configured_lines, changed_lines x ratio)`; a `docs_only` profile without that floor would collapse a documentation Story's budget to the flat configured number and make it exceed *more* often — the inverse of this Story's goal. `line_budget_ratio` therefore preserves the floor independently of the verdict, and the docs-only thresholds are relaxed on top of the risk-based profile (`max` of both line budgets and both floors), so `docs_only` is never stricter than what the same change would otherwise be held to. `budget.risk_profile_basis` records which risk profile it was relaxed over.
- Resolves the persistence depth to `summary` for a docs-only change, as a *default* and not an override. Precedence, highest first: (1) an explicit `--evidence-depth` request; (2) the pre-existing risk escalation — a high-risk profile or an active trigger signal (missing artifact, merge warning, waived or blocked gate) keeps the depth it would have had before this change; (3) the docs-only default. Ordering docs-only above the escalation would downgrade exactly the cases that need evidence most, so it is ordered below it.
- Classifying a change as docs-only never changes `risk_profile` or `trigger_signals`; both are recorded exactly as before, so the escalation input stays visible in the bundle.
- **Residual reach of the depth default.** `collectCanonicalAuditTriggerSignals` raises a signal for any missing artifact, any merge warning, and any gate node in `needs_review` / `needs_evidence` / `block` / `failed`. Across the 126 bundles promoted to date, 117 carry at least one such signal. The docs-only `summary` default therefore applies only to a documentation change promoted with a complete, fully-resolved artifact set; every other case keeps its escalated depth by design. The measurable win for the common case is the budget-scope split and the diff-base fix, not the depth label. Widening the depth default would require narrowing what counts as a risk signal, which is out of this Story's boundary.
