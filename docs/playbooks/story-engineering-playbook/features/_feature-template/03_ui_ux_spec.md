...
UI primitive、レイアウト、入力欄など、見た目や操作を変更する場合だけ記録します。

### 4.1 既存実装の探索

| 確認対象 | 探索した場所・検索語 | 採用する実装 | 判断理由 |
|---|---|---|---|
| 共通UI primitive |  |  |  |
| 類似画面・利用箇所 |  |  |  |

- 標準部品や既存の類似実装がある場合は、原則としてそれを利用する
- 独自primitiveを採用する場合は、既存部品では満たせない具体的な契約を判断理由に残す
- 探索結果が空のまま実装へ進まない
...
| 項目 | 記録 |
|---|---|
| ユーザー指摘を再現する対象viewport |  |
| 修正前スクリーンショット |  |
| 修正後スクリーンショット |  |
| スクリーンショットを見て改善したと判断した理由 |  |

- `overflow: 0`、DOM寸法、自動テスト成功だけでは視覚品質の証明にしない
- ユーザー指摘を対象viewportで再現し、修正後スクリーンショットを目視評価するまで、完了・PR準備・配備へ進めない
- CSS調整を2回行っても解消しない場合は追加の微調整を止め、DOM構造または標準部品の選択から再検討する
- この判断ノードはUI変更時だけの小さな確認であり、旧Gate DAG、強制並列レビュー、証跡収集ループを要求しない
...
- prompt、外部Design System、画像、HTML案はvisual hypothesisです。判断はStory、Spec、Architecture、現行route code、VibePro-native Design System、現在の検証結果を優先してください
