...
npx vibepro verify flow /path/to/repo --base-url http://127.0.0.1:3000 --id story-internal-beta
...
- `.vibepro/verification/<run-id>/`: Playwright実行ログ、生成spec、スクリーンショット、Flow Verification証跡
...
```bash
node bin/vibepro.js verify flow /path/to/repo \
  --base-url http://127.0.0.1:3000 \
  --id story-local-hardening
```

`verify flow` は対象リポジトリのPlaywright実行環境を使って、Storyの動線証跡を `.vibepro/verification/<run-id>/` に保存する。VibePro自身は開発サーバーを暗黙起動しないため、`--base-url` には起動済みアプリのURLを渡す。

Playwright依存やブラウザ本体が不足している場合は `needs_setup` として証跡を残し、次に実行すべき `npm install -D @playwright/test` や `npx playwright install chromium` を `setup.next_commands[]` に出す。

Basic認証付きのpreviewや本番確認では、パスワードをartifactに残さないためにenv経由を推奨する。

```bash
export SENPAI_BASIC_AUTH='user:password'
node bin/vibepro.js verify flow /path/to/repo \
  --base-url http://54.221.232.92 \
  --basic-auth-env SENPAI_BASIC_AUTH
```

一時的な用途では `--basic-auth user:password` や、`http://user:password@example.test` 形式の `--base-url` も扱える。保存される `flow-verification.json` にはユーザー名と認証sourceだけを残し、パスワードは保存しない。

生成される主な成果物:

```text
.vibepro/verification/<run-id>/
├── flow-verification.json
├── flow-verification.md
├── flow-verification.spec.js
├── playwright-output.log
└── screenshots/
```

`.vibepro/config.json` の `flow_design.runtime_probes[]` に `path`、`mutates`、`steps[]` を定義すると、そのプローブを実行する。`mutates: true` のプローブは `--allow-mutation` なしではskipする。`steps[]` は `expectVisible`、`expectNotVisible`、`click`、`fill`、`fillFromText`、`screenshot` を扱う。`fillFromText` は画面本文から正規表現で値を拾い、認証キーのような実行時表示値を入力できる。最新のFlow Verificationは `pr prepare` のE2E Gateへ接続され、PR本文の `Flow Verification Evidence` に記録される。
