---
name: diagnose-static-site
description: 静的サイトの一括診断。セキュリティチェック・デプロイ計画・リスク台帳・見積もり作成を順番に実行し、総合評価と商用化判定を行う。
---

# 静的サイト一括診断

指定したディレクトリの静的サイトコードに対して、セキュリティチェック・デプロイ計画・リスク台帳・見積もり作成を一括で実行する。
